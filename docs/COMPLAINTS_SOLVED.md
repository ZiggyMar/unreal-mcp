...
| # | Complaint | Source | Status | What this project does |
| --- | --- | --- | --- | --- |
| C1 | The editor hard-crashes on asset/node creation. | [ChiR24 #499](https://github.com/ChiR24/Unreal_mcp/issues) | **Partly** | Node creation goes through `unreal_find_node`'s reflection-verified catalog, so an unknown name fails with `didYouMean` rather than reaching the engine with garbage. One crash of exactly this family was then found in **this** project by live verification and fixed: `create_blueprint` checked the disk for a name collision (`DoesPackageExist`) while the engine asserts on memory, so create-after-delete in one session closed the editor. All four create paths now check memory first and return `asset_name_in_use`. Covered by a regression check in `npm run verify:live`. Still no fuzz suite over every spawnable node type. |
