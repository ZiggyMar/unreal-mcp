...
| # | Complaint | Source | Status | What this project does |
| --- | --- | --- | --- | --- |
| D1 | "Your MCP server is eating your context window": tool definitions alone consume the budget before the user's message is read. | [apideck](https://www.apideck.com/blog/mcp-server-eating-context-window-cli-alternative), [Albato on MCP context bloat](https://albato.com/blog/publications/embedded-mcp-context-bloat-hallucinations) | **Open** | 37 tools with deliberately instructive descriptions is a real up-front cost. The gateway/namespaced-tool pattern is in the backlog and is the right fix. Honest status: not solved. |
...
| # | Complaint | Source | Status | What this project does |
| --- | --- | --- | --- | --- |
| F1 | "Blueprint spaghetti that spirals out of control" — AI output compiles but no human can read it. | [StraySpark, Aura vs MCP 2026](https://www.strayspark.studio/blog/aura-vs-mcp-ai-assistants-unreal-engine-2026) | **Partly** | `unreal_organize_graph` provides node comments, comment boxes, and positions, and `unreal_build_graph` places a whole graph in one atomic call so layout can be planned rather than accreted. What is missing is that nothing *enforces* their use: a model can still emit a functional pile at the origin. Auto-layout and a lint pass are the open half. |
| F2 | Generated graphs have no comments, default positions, no grouping, no naming discipline. | Same, and this project's own M7 milestone | **Partly** | Same as F1: the capability exists, the enforcement does not. |
