...
| F2 | Generated graphs have no comments, default positions, no grouping, no naming discipline. | Same, and this project's own M7 milestone | **Partly** | Positions and grouping are solved: `unreal_auto_layout_graph` also wraps each execution chain in a comment box titled after its event, idempotently. Still open: nothing enforces *naming* discipline, and per-node explanatory comments are still up to the model. |
